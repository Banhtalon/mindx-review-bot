...
import {
  ...
  verifyManifest,
  writeJson,
} from "../scripts/qq-ai-workflow/lib/workflow.mjs";
import rules from "../.ai-workflow/RISK_RULES.json" with { type: "json" };
import { execFileSync } from "node:child_process";
import projectProfile from "../.ai-workflow/PROJECT_PROFILE.json" with { type: "json" };
...
test("pilot keeps routing manual and product workflows unchanged", () => {
  assert.equal(projectProfile.routing_mode, "MANUAL");
  assert.equal(projectProfile.attempt_ladder.at(-1), "technical-operator-stop");
  const changed = execFileSync(
    "git",
    ["diff", "--name-only", "255ccf9635aecc50474a0a88049355ef4c3638fc", "--", ".github/workflows"],
...
import { fixture, saveControl } from "../scripts/qq-ai-workflow/lib/pilot-fixture.mjs";
import { git, readControl, reserveAttempt } from "../scripts/qq-ai-workflow/lib/control.mjs";
