...
test("MindX profile binds v9.1.2 file-first roles manually to the active task", async () => {
  const profile = JSON.parse(await read(".ai-workflow/PROJECT_PROFILE.json"));
  ...
  assert.equal(profile.control_plane.current_issue, "https://github.com/Banhtalon/mindx-review-bot/issues/11");
  ...
  assert.equal(profile.manual_bindings.technical_operator, "chatgpt-work-or-qualified-human");
